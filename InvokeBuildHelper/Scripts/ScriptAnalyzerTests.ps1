...
Describe 'Script Analyzer' {
    ...
    foreach ($currentRule in $rule)
    {
        Context "$($currentRule.RuleName) ($($currentRule.Severity))" {
            ...
            $currentIssues =
                $issues |
                    Where-Object { $_.RuleName -eq $currentRule.RuleName } |
                        ForEach-Object { @{ RelativePath = '\' + $_.ScriptPath.Replace($BuildRoot, '').TrimStart('\'); Line = $_.Line; Column = $_.Column; Message = $_.Message } } |
                            Where-Object { $relativePath = $_.RelativePath; @($ExcludePath | Where-Object { $relativePath -like $_ }).Count -eq 0 }
            ...
            {
                It 'Should pass the rule for <RelativePath>:<Line>,<Column>' -ForEach $currentIssues {
            ...
            # $currentIssueCount = @($currentIssues).Count

            # if ($currentIssueCount -eq 0)
            # {
            #     It 'Should pass the rule for all files' {
